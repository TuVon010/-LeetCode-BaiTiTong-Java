...
- **本质理解** — 一句话定义核心思想
- **应用场景分类表** — 什么时候用、怎么用
- **Java 集合/数据结构对比** — HashMap vs TreeMap vs LinkedHashMap vs HashSet
- **可运行的 Java 代码模板** — 直接复制使用
- **模板变体及优化** — 覆盖常见变形

### 纠错文档（易错点追踪）

`05-纠错文档/` 记录刷题过程中犯过的每一个错误，防止"一错再错"：

- **错误总表** — 每个错误分配唯一编号（ERR-XXX），跟踪发现日期与状态
- **错误卡片** — 错误写法 → 正确写法 → 原因 → 记忆口诀，一目了然
- **间隔复习检验** — 基于艾宾浩斯遗忘曲线，在 +0/+1/+3/+7/+14 天五个时间点自动检验，连续 2 次通过才算掌握，复发则降级重练
- **自测题** — 每个错误配套自测题，随时自查
...
1. **对话前** — Claudian 自动读取 `学员档案`、`学习日志`、`进度看板`，AI 了解你的当前状态和薄弱环节；同时检查 `05-纠错文档`，对"到期"的易错点进行间隔复习检验
2. **教学中** — AI 用框架思维引导思路，遇卡壳先给提示不直接给答案，根据你的掌握程度动态调整难度
3. **对话后** — Claudian 自动将学习记录写回 `学习日志`、更新 `进度看板`、填充当日 `学习笔记`、登记当天新犯的错误到 `纠错文档`
...
项目内置了 **AI 自动上传** 规则（定义于 `CLAUDE.md`）：

- **自动触发** — AI 每次修改 Vault 文件后，自动执行 `git add -A` + `git commit` + `git push`，将最新学习进度同步到 GitHub
- **手动触发** — 对 AI 说「更新 / 上传 / 推一下」，立即同步
