...
- **去重时机：** 外层去重要在**进入循环时**判断（`if (i > 0 && nums[i] == nums[i-1]) continue;`），而不是找到答案后再去重。内层去重要在**找到答案后**进行。
...
List<List<Integer>> nSum(int[] nums, int target, int n) {
    /*
     * 通用 N 数之和模板（递归版）
     */
    Arrays.sort(nums);
...
List<List<Integer>> nSumHelper(int[] nums, long target, int n, int start) {
    List<List<Integer>> res = new ArrayList<>();
    if (n == 2) {
        // 两数之和：对撞指针
        ...
        for (int i = start; i <= nums.length - n; i++) {
            if ((long) nums[i] * n > target) break;  // 剪枝优化
            if (i > start && nums[i] == nums[i - 1]) continue;  // 去重
            List<List<Integer>> subRes = nSumHelper(nums, target - nums[i], n - 1, i + 1);
