...
class Solution {
    ...
    private void backtrack(String s, int start, List<String> path,
        ...
        for (int end = start; end < s.length(); end++) {
            if (isPalindrome(s, start, end)) {  // 实时判断
                path.add(s.substring(start, end + 1));
...
1. **切割点枚举范围**：`for (int end = start; end < n; end++)` 中 `end` 从 start 到 n-1，表示子串 `s.substring(start, end+1)`。越界是常见错误。
...
5. **substring 效率**：`s.substring(start, end+1)` 创建新子串，O(k) 时间。如果十分在意性能，可以传 start 和 end 索引到最终再切。
...
void backtrack(String s, int start, List<String> path, List<List<String>> res) {
    ...
    for (int end = start; end < s.length(); end++) {
        if (满足条件(s.substring(start, end + 1))) {
            path.add(s.substring(start, end + 1));
            backtrack(s, end + 1, path, res);  // 切割剩余部分
            path.remove(path.size() - 1);
        }
    }
}
