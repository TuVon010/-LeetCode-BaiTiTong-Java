...
**数据范围分析：**
- 矩阵最大 10x10 = 100 个元素，非常小，任何合法算法都不会超时。但作为经典题，O(mxn) 是标准解法。
- 元素值范围较小，不重要。

**输入输出特征：**
- 输入是 mxn 矩阵，输出是一维数组（螺旋顺序）。
- 螺旋顺序：-> 右 (列增加) -> v 下 (行增加) -> <- 左 (列减少) -> ^ 上 (行减少) -> 循环。
...
**复杂度：** O(m x n) 时间，O(1) 额外空间（输出数组不计）。
...
class Solution {
    public List<Integer> spiralOrder(int[][] matrix) {
        ...
        for (int k = 0; k < m * n; k++) {
            ...
            // 计算下一步
            int nr = row + dirs[dirIdx][0];
            int nc = col + dirs[dirIdx][1];
            
            // 如果下一步越界或已访问，转向
            if (nr < 0 || nr >= m || nc < 0 || nc >= n || visited[nr][nc]) {
                dirIdx = (dirIdx + 1) % 4;
                nr = row + dirs[dirIdx][0];
                nc = col + dirs[dirIdx][1];
            }
...
**复杂度：** O(m x n) 时间，O(m x n) 空间（visited 数组）。
...
class Solution {
    public List<Integer> spiralOrder(int[][] matrix) {
        List<Integer> res = new ArrayList<>();
        addLayer(matrix, 0, matrix.length - 1, 0, matrix[0].length - 1, res);
        return res;
    }
    
    private void addLayer(int[][] matrix, int t, int b, int l, int r, List<Integer> res) {
        if (t > b || l > r) {
            return;
        }
        // 上边：左 -> 右
        ...
        // 递归处理内层
        addLayer(matrix, t + 1, b - 1, l + 1, r - 1, res);
    }
}
```
...
- **for 循环的步长**：从右到左遍历时 `for (int j = right; j >= left; j--)`，从下到上遍历时 `for (int i = bottom; i >= top; i--)`。
...
- [[59-螺旋矩阵 II]] -- 本题的逆向问题：给定 n，生成一个 nxn 的螺旋矩阵。同样用边界收缩法，但方向是赋值而不是读取。
