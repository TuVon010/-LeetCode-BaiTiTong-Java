...
> **专题归类：** [[02-Wiki/专题总结/数组]] . [[02-Wiki/专题总结/动态规划]]
...
`sum` 只关心**以 `nums[i]` 结尾**这一段怎么选：

```java
sum = Math.max(sum + nums[i],   // 方案A：接上前面那段（延续之前的子数组）
               nums[i]);         // 方案B：从自己重开（丢弃前面的拖累）
```

```java
class Solution {
    public int maxSubArray(int[] nums) {
        // curSum: 以当前位置结尾的最大子数组和
        // maxSum: 全局最大子数组和
        int curSum = nums[0];
        int maxSum = nums[0];
...
**关键区别：** 贪心法在 `cur_sum < 0` 时重置为 0（相当于丢弃），而 DP 法用 `Math.max()` 选择是否重新开始。两者本质相同。
...
class Solution {
    ...
    private int divideAndConquer(int[] nums, int l, int r) {
        ...
        int crossMax = nums[mid] + leftMax + rightMax;
        
        // 取三者最大值
        int leftResult = divideAndConquer(nums, l, mid - 1);
        int rightResult = divideAndConquer(nums, mid + 1, r);
        return Math.max(Math.max(leftResult, rightResult), crossMax);
    }
}
```
...
- **初始化问题**：`maxSum` 必须初始化为 `nums[0]` 而不是 0。如果全为负数但初始化为 0 会返回错误结果 0。
- **空数组返回**：题目说至少一个元素，但如果是空数组需要特殊处理。
- **重置时机**：贪心法中重置 `curSum = 0` 而不是重置为当前元素值——因为下一轮循环会先加上当前元素。
