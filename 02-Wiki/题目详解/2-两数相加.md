...
**关键洞察：** 循环条件用 `while (l1 != null || l2 != null || carry != 0)` 可以统一处理三个情况：链表还有节点、链表遍历完了但还有进位。当某个链表遍历完时，对应的 val 视为 0。
...
**思路讲解：** 先遍历较短的链表，在其末尾补 0 节点使其与长链表等长，然后同步遍历相加。这样写循环时不需要每次都判断 null，但会修改原链表结构。
...
class Solution {
    public ListNode addTwoNumbers(ListNode l1, ListNode l2) {
        ...
        // 给短的补零
        ListNode dummyShort = new ListNode(0);
        ListNode cur = dummyShort;
        if (len1 < len2) {
...
- **循环条件：** 用 `while (l1 != null || l2 != null || carry != 0)` 而不是 `while (l1 != null && l2 != null)`，前者能处理长度不等的进位情况
- **取 val 时判空：** `(l1 != null) ? l1.val : 0` 而不是直接 `l1.val`，因为 l1 可能已经为空
- **链表前进：** 只有当前节点不为空时才移动指针，否则会触发 NullPointerException
