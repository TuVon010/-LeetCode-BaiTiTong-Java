...
class Solution {
    public List<List<Integer>> levelOrder(TreeNode root) {
        ...
        while (!q.isEmpty()) {
            ...
            int levelSize = q.size();                   // 当前层的节点数（关键！）
            for (int i = 0; i < levelSize; i++) {
                TreeNode node = q.removeFirst();
                level.add(node.val);                    // 访问当前节点
                if (node.left != null) {
                    q.addLast(node.left);               // 左孩子入队
                }
                if (node.right != null) {
                    q.addLast(node.right);              // 右孩子入队
                }
            }
            res.add(level);                             // 将当前层加入结果
        }
...
class Solution {
    ...
    private void dfs(TreeNode node, int depth, List<List<Integer>> res) {
        ...
        res.get(depth).add(node.val);      // 放入对应层的列表中
        dfs(node.left, depth + 1, res);    // 递归左子树
        dfs(node.right, depth + 1, res);   // 递归右子树
    }
}
```
...
- **BFS 中队列长度必须在 `for` 循环前固定：** 如果在 for 循环中动态取队列长度，由于循环中会出队和入队，长度会不断变化。必须在一开始就把长度存到变量中。
- **DFS 中 `res` 的初始化顺序：** 当第一次进入第 depth 层时，必须先 `res.add(new ArrayList<>())`，然后再赋值。如果 res 的长度 <= depth，说明这一层还没创建。
- **空树返回 `[]` 而不是 `[[]]`：** 空树没有节点，所以返回空列表而不是包含一个空列表。
- **BFS 使用 `ArrayDeque` 的 `removeFirst()`：** Java 中用 `Deque` 实现队列，`removeFirst()` 是 O(1)。
...
**变体思路：** 将 `for (int i = 0; i < levelSize; i++)` 中的 `i` 可以用于判断当前节点在层中的位置（如右视图问题中判断 `i == levelSize - 1`）。
