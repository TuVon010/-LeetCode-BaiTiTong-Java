...
**数据范围分析：**
- 矩阵最大 200x200 = 40,000 个元素，暴力算法 O(mxnx(m+n)) 约 1.6x10^7 还可以接受，但显然 O(mxn) 才是标准解法。
...
**复杂度：** O(m x n) 时间，O(1) 空间。

### 思路二：用额外标记数组（更易理解，O(m+n) 空间）

用两个布尔数组 `rowZero` 和 `colZero` 分别记录哪些行和列需要置零。先扫描矩阵做标记，再根据标记置零。
...
public void setZeroes(int[][] matrix) {
    ...
    for (int j = 0; j < n; j++) if (matrix[0][j] == 0) firstRowFlag = true;
    for (int i = 0; i < m; i++) if (matrix[i][0] == 0) firstColFlag = true;
    
    // Step 2: 用首行首列标记其他区域
    for (int i = 1; i < m; i++) {
        for (int j = 1; j < n; j++) {
            if (matrix[i][j] == condition) {
                matrix[i][0] = mark;  // 标记行
                matrix[0][j] = mark;  // 标记列
            }
        }
    ...
    for (int i = 1; i < m; i++) {
        for (int j = 1; j < n; j++) {
            if (matrix[i][0] == mark || matrix[0][j] == mark) {
                matrix[i][j] = modifiedValue;
            }
        }
    }
    
    // Step 4: 处理首行首列
    if (firstRowFlag) for (int j = 0; j < n; j++) matrix[0][j] = modifiedValue;
    if (firstColFlag) for (int i = 0; i < m; i++) matrix[i][0] = modifiedValue;
}
```
