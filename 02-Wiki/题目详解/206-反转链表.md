...
- **核心约束：** 反转后原链表的尾节点变成新链表的头节点，原头节点变成新链表的尾节点（next 指向 null）
- **隐藏条件：** 链表的单向性意味着一旦断开 next 连接就会丢失后续节点，操作前必须先保存
- **边界条件：** 空链表（head 为 null）、只有一个节点的链表
...
具体怎么做呢？先准备好一根空线（pre = null），然后从第一颗珠子开始，每取下一颗珠子，就把它串到新线的最前面。这样取完所有珠子后，新线上珠子的顺序就是反的了。
...
class Solution {
    public ListNode reverseList(ListNode head) {
        /* 迭代法：三指针翻转 */
        ListNode pre = null;   // 已反转部分的头
        ListNode cur = head;   // 当前待反转节点
        while (cur != null) {
            ListNode nxt = cur.next;   // 先保存下一个节点，防断链
...
**思路讲解：** 递归地反转 `head.next` 及之后的链表，得到的新链表头节点为 `newHead`。此时 `head.next` 指向反转后的最后一个节点，只需让 `head.next.next = head` 将当前节点接到尾部，再让 `head.next = null` 断掉原连接。递归的终止条件是空或只有一个节点。
...
- **递归返回值：** 递归的核心是返回 newHead（即原链表尾节点），不是 head
- **尾节点处理：** 反转后原头节点变成尾节点，要记得将 `head.next` 设为 null，否则形成环
- **空链表和单节点：** 这两种情况直接返回 head，反转前和反转后结果一样
- **pre 初始化：** pre 必须初始化为 null，因为反转后头节点的 next 指向 null
