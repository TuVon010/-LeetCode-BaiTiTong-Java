...
class Solution {
    public List<Integer> findAnagrams(String s, String p) {
        ...
        for (int i = 0; i < p.length(); i++) {
            count[p.charAt(i) - 'a']--;  // 先减去目标
        }

        List<Integer> res = new ArrayList<>();
        int diff = 0;  // 不为 0 的字符种类数
        for (int i = 0; i < 26; i++) {
            if (count[i] != 0) {
                diff++;
            }
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            // 移除离开窗口的字符
            if (i >= p.length()) {
                int idxOut = s.charAt(i - p.length()) - 'a';
                if (count[idxOut] == 0) {
                    diff++;  // 即将变成非 0
                } else if (count[idxOut] == 1) {
                    diff--;  // 即将变成 0
                }
                count[idxOut]--;
            }
...
- **p 比 s 长：** 直接返回空列表，否则后续的 `s[i - p.length()]` 会取到负索引（在 Java 中会抛出 StringIndexOutOfBoundsException）。一定要先检查。
- **计数数组索引计算：** `ch - 'a'`，不是 `ch` 本身，也不是 `ch - 'a' + 1`（'a' 的 ASCII 码是 97）。
- **窗口形成判断：** `if (i >= p.length())` 表示当索引 i 超过等于 p 长度时，窗口已满，需要移除左边界元素。注意是 `>=` 不是 `>`。
- **结果索引计算：** `i - p.length() + 1`，不是 `i` 也不是 `i + 1`。例如 p 长度为 2，当 i=1 时窗口为 [0,1]，起始索引是 0 = 1 - 2 + 1。
- **数组比较：** `cnt_s == cnt_p` 在 Java 中是比较引用而非内容，必须用 `Arrays.equals(cntS, cntP)`。
...
class Solution {
    public List<Integer> fixedWindow(String s, ...) {
        /*
         * 定长滑动窗口通用模板
         */
        int n = s.length();
        ...
        if (k > n) {
            return new ArrayList<>();  // 或相应默认值
        }

        // 初始化窗口状态
        ... windowState = initState(...);
        ... targetState = initTarget(...);
        List<Integer> res = new ArrayList<>();

        for (int i = 0; i < n; i++) {
            // 1. 加入右指针元素
            updateState(windowState, s.charAt(i), +1);

            // 2. 移除左指针元素（窗口满时）
            if (i >= k) {
                updateState(windowState, s.charAt(i - k), -1);
            }

            // 3. 判断窗口状态是否满足条件
            if (i >= k - 1 && matches(windowState, targetState)) {
                res.add(i - k + 1);  // 记录起始索引
            }
        }
