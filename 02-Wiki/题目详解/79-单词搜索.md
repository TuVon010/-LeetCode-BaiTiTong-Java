...
**核心思想：** 从每个格子出发，进行深度优先搜索（DFS）。用特殊字符（如 `#`）临时覆盖格子值来标记已访问（省去 visited 数组），回溯时恢复。
...
class Solution {
    ...
    private boolean dfs(char[][] board, boolean[][] visited, String word, int i, int j, int k) {
        ...
        visited[i][j] = true;
        boolean found = (dfs(board, visited, word, i + 1, j, k + 1) ||
...
2. **四方向顺序优化**：先搜索最可能的方向（虽然没有通用规则），但短路或 `||` 天然带剪枝——找到一个就返回。
3. **索引越界检查顺序**：必须先检查越界再访问 board。写成 `board[i][j] != word.charAt(k) or 越界` 会先下标越界报错。
4. **早期剪枝遗漏**：如果 word 长度 > m x n，直接返回 false。这是简单有效的早期剪枝。
5. **Java char 数组原地标记**：`board[i][j] = '#'` 直接修改原数组是允许的，但要确保恢复。
...
boolean dfs(int i, int j, int k) {
    if (达到目标状态) return true;
    if (越界 || 不匹配 || 已访问) return false;
    
    标记已访问;
    for each 方向 in [上, 下, 左, 右]:
        if (dfs(新坐标, 新状态)) return true;  // 找到即返回
    恢复标记;
    return false;
}
```
...
- 因为只需要找一条路径，短路返回（found 用 || 连接）可大幅减少搜索量。
