...
class Solution {
    public int longestCommonSubsequence(String text1, String text2) {
        ...
        for (int i = 1; i <= m; i++) {
            int prev = 0;                           // 左上角 dp[i-1][j-1]
            for (int j = 1; j <= n; j++) {
                int temp = dp[j];                   // 保存当前值（将成为下一轮的左上角）
                if (text1.charAt(i - 1) == text2.charAt(j - 1)) {
                    dp[j] = prev + 1;               // 左上角 + 1
                } else {
                    dp[j] = Math.max(dp[j], dp[j - 1]);  // 上方 vs 左方
                }
                prev = temp;                        // 更新左上角
            }
        }
...
class Solution {
    ...
    private int dfs(String text1, String text2, int i, int j, int[][] memo) {
        ...
        if (text1.charAt(i - 1) == text2.charAt(j - 1)) {
            ...
            memo[i][j] = Math.max(dfs(text1, text2, i - 1, j, memo),
...
- **索引偏移**：`text1.charAt(i-1)` 对应 `dp[i]`（前 i 个字符）。字符串索引和 dp 索引差 1。
- **遍历顺序不可颠倒**：必须先处理 text1 再处理 text2（或反过来都可以，但要一致）。不能提前用 dp[i-1][j-1] 的未初始化值。
...
```java
int twoStringDP(String text1, String text2) {
    int m = text1.length(), n = text2.length();
    int[][] dp = new int[m + 1][n + 1];
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            if (text1.charAt(i - 1) == text2.charAt(j - 1)) {
                ...
                dp[i][j] = Math.max(dp[i - 1][j], dp[i][j - 1]);  // 不等则跳过
            }
        }
    }
