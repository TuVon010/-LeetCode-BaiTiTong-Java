...
class Solution {
    public boolean canPartition(int[] nums) {
        ...
        for (int num : nums) {
            for (int j = target; j >= num; j--) {  // 倒序遍历，0-1 背包
                dp[j] = dp[j] || dp[j - num];      // 不选 or 选
            }
        }
...
class Solution {
    public boolean canPartition(int[] nums) {
        ...
        int bits = 1;  // 初始化为 1，表示 dp[0] = True
        for (int num : nums) {
...
class Solution {
    ...
    private boolean dfs(int[] nums, int idx, int remain) {
        if (remain == 0) {
            return true;
        }
        ...
        // 选择当前元素 或 跳过当前元素
        return dfs(nums, idx + 1, remain - nums[idx]) || dfs(nums, idx + 1, remain);
    }
}
```
...
```java
class Solution {
    public boolean zeroOneKnapsackFeasible(int[] nums, int target) {
        boolean[] dp = new boolean[target + 1];
        dp[0] = true;

        for (int num : nums) {                       // 遍历物品
            for (int j = target; j >= num; j--) {    // 倒序遍历容量
                dp[j] = dp[j] || dp[j - num];        // 不选或选
            }
        }
