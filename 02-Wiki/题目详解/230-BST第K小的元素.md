...
class Solution {
    ...
    private void inorder(TreeNode node, int k) {
        ...
        inorder(node.left, k);          // 左：先遍历左子树（更小的元素）

        count++;                          // 根：访问当前节点，计数器 +1
        if (count == k) {
            result = node.val;
            return;                       // 找到后提前返回
        }

        inorder(node.right, k);         // 右：遍历右子树
    }
}
```
...
class Solution {
    public int kthSmallest(TreeNodeWithSize root, int k) {
        /* 假设树节点已经维护了 leftSize 属性 */
        TreeNodeWithSize cur = root;
        while (cur != null) {
            if (k <= cur.leftSize) {
                cur = cur.left;
            } else if (k == cur.leftSize + 1) {
                return cur.val;
            } else {
                k -= cur.leftSize + 1;
                cur = cur.right;
...
- **迭代法 `k--` 的位置：** 必须在弹出栈顶（访问节点）时减 1，而不是入栈时减 1。
