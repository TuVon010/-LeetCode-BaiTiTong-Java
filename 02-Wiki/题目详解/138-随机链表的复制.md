...
class Solution {
    public Node copyRandomList(Node head) {
        ...
        // 第三步：拆分链表
        cur = head;
        Node newHead = head.next;  // 保存克隆链表的头
        while (cur != null) {
            Node clone = cur.next;
...
**思路讲解：** 用哈希表建立「原节点 → 克隆节点」的映射。第一遍遍历创建所有克隆节点并建立映射；第二遍遍历根据映射设置克隆节点的 next 和 random 指针。思路直观，但需要 O(n) 空间。
...
class Solution {
    public Node copyRandomList(Node head) {
        ...
        Map<Node, Node> mapping = new HashMap<>();  // 原节点 → 克隆节点
...
- **random 为 null 的情况：** `cur.random.next` 会崩溃，必须先判断 `if (cur.random != null)`。哈希表法中用 `mapping.get(cur.random)` 返回 null 处理
