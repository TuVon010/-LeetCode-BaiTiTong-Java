...
**处理边界：**
- 当 i = 0 时，nums1 左半没有元素，用 `Integer.MIN_VALUE` 表示左边最大值。
- 当 i = m 时，nums1 右半没有元素，用 `Integer.MAX_VALUE` 表示右边最小值。
- j 同理。
...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        int left = 0, right = m;
        int totalLeft = (m + n + 1) / 2;  // 左半部分需要的元素总数
...
1. **边界值处理**：分割线在数组两端时，用 `Integer.MIN_VALUE` 和 `Integer.MAX_VALUE` 表示是优雅的处理方式。忘记处理边界会导致索引越界。
...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while (left <= right) {
            int i = left + (right - left) / 2;  // nums1 分割线
            int j = totalLeft - i;               // nums2 分割线
...
3. **极值处理边界**：用 `Integer.MIN_VALUE` / `Integer.MAX_VALUE` 优雅处理分割线在数组两端的情况。
