...
**核心洞察**：由于负数的存在，当前最大乘积可能来自之前的最大正数乘正数，也可能来自之前的最小负数乘负数。因此同时维护当前最大 `maxProd` 和当前最小 `minProd`。

**DP 五步法：**
1. **dp 定义**：`maxProd[i]` 表示以 i 结尾的子数组最大乘积，`minProd[i]` 表示最小乘积
2. **递推公式**：
   - `maxProd[i] = max(nums[i], maxProd[i-1] * nums[i], minProd[i-1] * nums[i])`
   - `minProd[i] = min(nums[i], maxProd[i-1] * nums[i], minProd[i-1] * nums[i])`
3. **初始化**：`maxProd[0] = minProd[0] = nums[0]`
4. **遍历顺序**：从左到右
...
class Solution {
    public int maxProduct(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            int num = nums[i];
            int candidate1 = maxProd * num;
            int candidate2 = minProd * num;
            maxProd = Math.max(num, Math.max(candidate1, candidate2));
            minProd = Math.min(num, Math.min(candidate1, candidate2));
            result = Math.max(result, maxProd);
        }
        return result;
    }
