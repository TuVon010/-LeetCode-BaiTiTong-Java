...
**第一步（预处理）：** 遍历一次字符串，用数组记录每个字符最后出现的下标。
...
class Solution {
    public List<Integer> partitionLabels(String s) {
        ...
        // 第二步：遍历字符串，贪心切割
        for (int i = 0; i < s.length(); i++) {
            end = Math.max(end, last[s.charAt(i) - 'a']);  // 不断扩展当前片段的右边界
...
```java
public List<Integer> partitionByLast(String s) {
    // 1. 预处理：记录每个字符的最后出现位置
    ...
    int start = 0, end = 0;
    for (int i = 0; i < s.length(); i++) {
        end = Math.max(end, lastPos[s.charAt(i) - 'a']);  // 扩展右边界
        if (i == end) {                  // 可切分
            ans.add(end - start + 1);
            start = i + 1;
        }
