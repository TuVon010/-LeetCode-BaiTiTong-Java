...
public int numSquares(int n) {
    ...
    for (int i = 1; i <= n; i++) {
        int j = 1;
        while (j * j <= i) {                              // 枚举完全平方数
            dp[i] = Math.min(dp[i], dp[i - j * j] + 1);   // 取最小值
            j++;
        }
    }
    return dp[n];
...
public int numSquares(int n) {
    Deque<int[]> queue = new ArrayDeque<>();         // (当前值, 步数)
    queue.offer(new int[]{n, 0});
    boolean[] visited = new boolean[n + 1];
    visited[n] = true;
    while (!queue.isEmpty()) {
        int[] cur = queue.poll();
        int num = cur[0], step = cur[1];
        int j = 1;
        while (j * j <= num) {
            int nxt = num - j * j;
            if (nxt == 0) {
...
- **inf 设置**：推荐 `Integer.MAX_VALUE` 或 `n + 1`，不要用 `10**9` 等可能导致相加溢出的值。
...
public int completeKnapsackMin(int[] coins, int amount) {
