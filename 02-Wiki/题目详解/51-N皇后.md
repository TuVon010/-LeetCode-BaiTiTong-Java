...
class Solution {
    public List<List<String>> solveNQueens(int n) {
        ...
        Set<Integer> cols = new HashSet<>();      // 已被占用的列
        Set<Integer> diag1 = new HashSet<>();      // 已被占用的主对角线 (row - col)
        Set<Integer> diag2 = new HashSet<>();      // 已被占用的副对角线 (row + col)
        
        backtrack(0, n, new ArrayList<>(), res, cols, diag1, diag2);
        return res;
    }
    
    private void backtrack(int row, int n, List<char[]> path, List<List<String>> res,
        ...
        for (int col = 0; col < n; col++) {
            ...
            backtrack(row + 1, n, path, res, cols, diag1, diag2);  // 递归下一行
...
class Solution {
    public List<List<String>> solveNQueens(int n) {
        ...
        boolean[] diag1 = new boolean[2 * n - 1];  // row - col 范围: -(n-1) 到 (n-1)
        boolean[] diag2 = new boolean[2 * n - 1];  // row + col 范围: 0 到 2n-2
    ...
    private void backtrack(int row, int n, List<char[]> path, List<List<String>> res,
        ...
        for (int col = 0; col < n; col++) {
            ...
            rowStr[col] = 'Q';
            path.add(rowStr);
            backtrack(row + 1, n, path, res, cols, diag1, diag2);
            path.remove(path.size() - 1);
...
2. **负数索引问题**：在集合版本中 `row - col` 可能是负数，但 Java 的 HashSet 也可以存储负数，这是集合版本比数组版本简单的地方。数组版本需要加偏移 `row - col + n - 1`。
3. **路径拷贝**：`res.add(new ArrayList<>(path))` 中 path 的每一行是 char 数组，需要转换为字符串并创建新列表，避免后续被修改。
...
```java
void solve(int boardSize) {
    List<Result> res = new ArrayList<>();
    Set<Integer> cols = new HashSet<>();
    Set<Integer> diag1 = new HashSet<>();  // row - col
    Set<Integer> diag2 = new HashSet<>();  // row + col

    void backtrack(int row, List<...> path) {
        if (row == boardSize) {
            记录结果;
            return;
        }
        for (int col = 0; col < boardSize; col++) {
            if (冲突(cols, diag1, diag2)) continue;
            标记(row, col);
            构建当前行 -> path;
