...
class Solution {
    ...
    private void backtrack(int[] candidates, int remaining, int start,
        ...
        if (remaining == 0) {
            res.add(new ArrayList<>(path));  // 找到一个合法组合
            return;
        }
        ...
        for (int i = start; i < candidates.length; i++) {
            ...
            backtrack(candidates, remaining - candidates[i], i, path, res);
...
void backtrack(int[] candidates, int remaining, int start,
    ...
    for (int i = start; i < candidates.length; i++) {
        ...
        path.add(candidates[i]);
        backtrack(candidates, remaining - candidates[i], i, path, res);  // 传 i 可重复选
        path.remove(path.size() - 1);
    }
}
```
