...
class Solution {
    ...
    private void backtrack(int left, int right, int n, StringBuilder path, List<String> res) {
        // 左右括号都用完了，得到一个有效组合
        if (left == n && right == n) {
        ...
        if (left < n) {
            path.append('(');
            backtrack(left + 1, right, n, path, res);
        ...
        if (right < left) {
            path.append(')');
            backtrack(left, right + 1, n, path, res);
            path.deleteCharAt(path.length() - 1);
        }
    }
}
...
class Solution {
    ...
    private void backtrack(int leftRem, int rightRem, StringBuilder path, List<String> res) {
        // 没有剩余括号了
        ...
        if (leftRem > 0) {
            path.append('(');
            backtrack(leftRem - 1, rightRem, path, res);
            path.deleteCharAt(path.length() - 1);
        }
        ...
        if (rightRem > leftRem) {
            path.append(')');
            backtrack(leftRem, rightRem - 1, path, res);
            path.deleteCharAt(path.length() - 1);
        }
    }
}
...
1. **右括号的约束条件写错**：`right < left`（已使用角度）或 `rightRem > leftRem`（剩余角度）。如果写成 `right <= left`，就会在 `right == left` 时允许放右括号，产生无效的 `())` 序列。
2. **忘记撤销操作**：Java 中使用 StringBuilder 需要显式撤销 `path.deleteCharAt(path.length() - 1)`。
...
```java
void backtrack(状态参数, StringBuilder 路径) {
    if (达到目标状态) {
        记录结果;
        return;
    }
