...
- **数据范围含义：** 最多 1000 个节点，节点值范围 [-10^9, 10^9]，可能非常大，累加可能超过 32 位整数范围，需注意累加可能溢出，使用 long 类型。
...
class Solution {
    ...
    private int dfs(TreeNode node, long curSum, int targetSum, Map<Long, Integer> prefix) {
        ...
        curSum += node.val;
        // 以当前节点结尾的满足条件的路径数
        int count = prefix.getOrDefault(curSum - targetSum, 0);

        // 记录当前前缀和
        prefix.put(curSum, prefix.getOrDefault(curSum, 0) + 1);
        // 递归处理左右子树
        count += dfs(node.left, curSum, targetSum, prefix);
        count += dfs(node.right, curSum, targetSum, prefix);
...
class Solution {
    public int pathSum(TreeNode root, int targetSum) {
        ...
        // 以每个节点为起点 + 递归处理左右子树
        return dfsFromNode(root, (long) targetSum) +
               pathSum(root.left, targetSum) +
               pathSum(root.right, targetSum);
    }

    // 以当前节点为起点，向下搜索
    private int dfsFromNode(TreeNode node, long target) {
        if (node == null) {
            return 0;
        }
...
- **数值溢出：** 节点值范围 [-10^9, 10^9]，1000 个节点累加可能达到 ±10^12，需要用 long 类型。
- **前缀和字典不要用数组替代：** 因为需要的是「出现次数」的累加，不是位置索引。
...
class Solution {
    ...
    private int dfs(TreeNode node, long curSum, int targetSum, Map<Long, Integer> prefix) {
        ...
        curSum += node.val;
        int count = prefix.getOrDefault(curSum - targetSum, 0);  // 关键：查找之前出现过的前缀和
        prefix.put(curSum, prefix.getOrDefault(curSum, 0) + 1);  // 记录当前前缀和
        count += dfs(node.left, curSum, targetSum, prefix);
        count += dfs(node.right, curSum, targetSum, prefix);
