...
class Solution {
    ...
    private void backtrack(String digits, int idx, StringBuilder path,
        ...
        for (char ch : letters.toCharArray()) {
            path.append(ch);                                          // 做选择
            backtrack(digits, idx + 1, path, res, mapping);          // 递归处理下一个数字
...
2. **数字与字符串混淆**：映射表的 key 是字符 `'2'` 而不是整数 `2`，用 `digits.charAt(idx)` 获取的也是字符。
3. **回溯终止条件**：`idx == digits.length()` 时记录结果，此时 `path` 的长度也等于 `digits.length()`。
...
```java
void backtrack(int level, List<Character> path, List<List<Character>> res) {
    if (level == totalLevels) {
        res.add(new ArrayList<>(path));  // 记录结果
        return;
    }

    List<Character> choices = getChoices(level);  // 当前层选择列表
    for (Character choice : choices) {
        path.add(choice);
        backtrack(level + 1, path, res);
