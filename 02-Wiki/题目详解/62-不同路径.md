...
- **数据范围**：m, n <= 100，结果在 32 位整数范围内（实际约 C(198,99) approx 9e55 会溢出 32 位，但题目保证不溢出）。
...
class Solution {
    public int uniquePaths(int m, int n) {
        int[] dp = new int[n];               // 第一行全为 1
        for (int j = 0; j < n; j++) {
            dp[j] = 1;
        }
        
        for (int i = 1; i < m; i++) {        // 遍历剩余行
            for (int j = 1; j < n; j++) {    // 每行从第 2 列开始
...
总步数 = (m-1)+(n-1) = m+n-2，选择其中 n-1 步向右走（或 m-1 步向下走）。使用 long 类型存储组合数结果。
...
class Solution {
    public int uniquePaths(int m, int n) {
        ...
        int chooseSteps = Math.min(m - 1, n - 1);  // 选较小的以减少计算
        
        for (int i = 1; i <= chooseSteps; i++) {
            result = result * (totalSteps - chooseSteps + i) / i;
        }
...
- **m, n 输入顺序**：m 是行数，n 是列数。创建数组时 `dp = new int[m][n]`。
- **组合数溢出**：使用 long 类型存储组合数结果，避免 int 溢出。
...
```java
public int gridPathCount(int m, int n) {
    int[][] dp = new int[m][n];
    // 初始化为 1（第一行第一列已正确）
...
```java
public int gridPathCountOptimized(int m, int n) {
    int[] dp = new int[n];
    Arrays.fill(dp, 1);
    for (int i = 1; i < m; i++) {
        for (int j = 1; j < n; j++) {
