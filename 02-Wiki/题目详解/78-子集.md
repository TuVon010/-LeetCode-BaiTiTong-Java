...
class Solution {
    ...
    private void backtrack(int start, int[] nums, List<Integer> path, List<List<Integer>> res) {
        ...
        for (int i = start; i < nums.length; i++) {
            path.add(nums[i]);                // 做选择
            backtrack(i + 1, nums, path, res); // 递归：只能选 i 之后的元素
...
class Solution {
    public List<List<Integer>> subsets(int[] nums) {
        ...
        for (int mask = 0; mask < (1 << n); mask++) {
            ...
            for (int i = 0; i < n; i++) {
                if ((mask & (1 << i)) != 0) {  // 第 i 位为 1 表示选中 nums[i]
                    subset.add(nums[i]);
                }
            }
            res.add(subset);
...
2. **忘记创建副本**：`res.add(new ArrayList<>(path))` 而不是 `res.add(path)`。`path` 随后会被修改，不创建副本会导致结果被覆盖。
...
void backtrack(int start, int[] nums, List<Integer> path, List<List<Integer>> res) {
    ...
    for (int i = start; i < n; i++) {
        path.add(nums[i]);                // 做选择
        backtrack(i + 1, nums, path, res); // 递归：只能向后选
        path.remove(path.size() - 1);     // 撤销选择
    }
}
```
