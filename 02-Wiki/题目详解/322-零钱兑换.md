...
class Solution {
    public int coinChange(int[] coins, int amount) {
        ...
        for (int coin : coins) {                         // 遍历物品（硬币）
            for (int j = coin; j <= amount; j++) {       // 正序遍历背包容量
...
class Solution {
    public int coinChange(int[] coins, int amount) {
        int ans = dfs(amount, coins, new HashMap<>());
...
class Solution {
    public int coinChange(int[] coins, int amount) {
        ...
        while (!queue.isEmpty()) {
            int[] cur = queue.poll();
            int curAmount = cur[0], step = cur[1];
            for (int coin : coins) {
                int nxt = curAmount - coin;
                if (nxt == 0) {
                    return step + 1;
                }
...
- **硬币面额大小**：coins[i] 可能大于 amount，遍历背包时 `for (int j = coin; j <= amount; j++)` 自动跳过。
...
class Solution {
    public int completeKnapsackMin(int[] coins, int target) {
        int[] dp = new int[target + 1];
        Arrays.fill(dp, Integer.MAX_VALUE);
        dp[0] = 0;
        for (int coin : coins) {                        // 遍历物品
            for (int j = coin; j <= target; j++) {      // 正序遍历容量
...
- 最小值：`dp[j] = Math.min(dp[j], dp[j - coin] + 1)`，初值 Integer.MAX_VALUE
