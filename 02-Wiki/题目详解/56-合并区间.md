...
- 相接 = 重叠（即 interval[0] <= merged 最后一个区间的终点时需要合并，包含等于情况）。
...
**代码实现技巧：** 利用 `merged` 列表中最后一个区间（即当前正在合并的区间），可以让代码非常简洁。
...
class Solution {
    public int[][] merge(int[][] intervals) {
        ...
        for (int[] inter : intervals) {
            events.add(new int[]{inter[0], 0});    // 0 表示起点
            events.add(new int[]{inter[1], 1});    // 1 表示终点
        }
...
- **更新终点**：合并时要取 `Math.max(end, interval[1])` 而不是直接用 `interval[1]`——因为新区间可能完全包含在当前合并区间内。
- **最后一个区间**：遍历结束后别忘了把最后一个合并区间加入结果。
- **空输入**：如果 `intervals` 为空，直接返回空数组。
...
public int[][] mergeIntervals(int[][] intervals) {
    ...
    for (int[] interval : intervals) {
        // 不重叠条件
        if (merged.isEmpty() || interval[0] > merged.get(merged.size() - 1)[1]) {
            merged.add(new int[]{interval[0], interval[1]});     // 新区间
        } else {
            // 重叠：合并（更新终点）
