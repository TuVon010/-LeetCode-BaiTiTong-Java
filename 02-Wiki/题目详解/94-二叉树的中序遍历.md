...
- **边界条件：** 空树（root 为 null）返回空列表 `[]`；只有一个节点直接返回该节点值。
...
class Solution {
    ...
    private void dfs(TreeNode node, List<Integer> res) {
        ...
        dfs(node.left, res);        // 左：递归遍历左子树
        res.add(node.val);          // 根：访问当前节点（中序位置）
        dfs(node.right, res);       // 右：递归遍历右子树
    }
}
```
...
- **忘记 base case：** 递归函数中必须先处理 `if (node == null) return`，否则会无限递归导致栈溢出。
