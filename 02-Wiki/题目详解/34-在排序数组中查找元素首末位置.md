...
**核心思想：** 使用 lower_bound（第一个 >= target）和 upper_bound_exclusive（第一个 > target）的组合来找到左右边界。
...
class Solution {
    public int[] searchRange(int[] nums, int target) {
        ...
        int right = upperBoundExclusive(nums, target) - 1;
        return new int[]{left, right};
    }
    ...
    private int upperBoundExclusive(int[] nums, int target) {
        int l = 0, r = nums.length - 1;
        while (l <= r) {
            int mid = l + (r - l) / 2;
            if (nums[mid] <= target) {
...
**注意：** 面试时理解 `lower_bound` / `upper_bound` 的内部实现是更重要的。
...
2. **验证逻辑**：必须检查 `left >= nums.length`（越界）或 `nums[left] != target`（值不存在），二者任一成立则返回 [-1, -1]。
3. **空数组**：`nums.length == 0` 时 `lowerBound()` 返回 0，检查 `left >= nums.length` 时触发，返回 [-1, -1]。
4. **右边界用 `upperBoundExclusive` 加等号的技巧**：如果记不住右边界模板，可以用"找第一个 > target 的位置，减 1 得右边界"代替。
...
**记忆技巧：**
- `lowerBound` vs `upperBound` 就差一个等号。
- 等号在左（`nums[mid] <= target`）→ 右边界（向左收缩）。
- 等号在右（`nums[mid] < target`）→ 左边界（向右收缩）。
- 最终 `lowerBound` 返回 `l`，`upperBound` 返回 `r`。
