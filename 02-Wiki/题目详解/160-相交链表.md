...
- **值相等不等于节点相交：** 相交判断的是节点引用（`pA == pB`），不是节点值相等（`pA.val == pB.val`）。两个不同节点可以有相同的值
- **空链表处理：** 任一链表为空时直接返回 null
- **无相交情况：** 等路程法在无相交时会同时到达 null，返回 null，不需要特殊判断
- **死循环风险：** 等路程法的 while 循环条件用 `pA != pB`，如果链表不相交且代码写错（没有换路逻辑），会产生死循环
- **交换赛道的时机：** 是 `pA = (pA != null) ? pA.next : headB`，而不是 `pA = (pA.next != null) ? pA.next : headB`。前者在 pA 为 null 时换路，后者会跳过最后一个节点
