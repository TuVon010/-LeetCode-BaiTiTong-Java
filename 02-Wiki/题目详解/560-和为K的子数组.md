...
**数据范围分析：**
- 数组长度最大 2x10^4，O(n^2) 的枚举法（约 4 亿次操作）会超时，必须 O(n) 或 O(n log n) 解决。
- 元素值可正可负可为 0，这意味着不能用双指针（滑动窗口），因为双指针要求单调性，而负数会导致和不是单调的。
- k 的范围很大（plusmn10^7），但哈希表存的是前缀和的值，与 k 大小无关。
...
- 前缀和可能非常大（n x max(nums) = 2x10^4 x 1000 = 2x10^7），仍在 32 位整数范围内。
...
class Solution {
    public int subarraySum(int[] nums, int k) {
        ...
        for (int num : nums) {
            curSum += num;                 // 更新前缀和
            // 如果存在 curSum - k，说明有子数组的和为 k
            count += prefix.getOrDefault(curSum - k, 0);
            // 将当前前缀和存入哈希表
...
class Solution {
    public int subarraySum(int[] nums, int k) {
        int n = nums.length;
        int[] prefix = new int[n + 1];  // prefix[i] 表示前 i 个元素的和
        for (int i = 0; i < n; i++) {
        ...
        for (int i = 0; i <= n; i++) {
            count += hashMap.getOrDefault(prefix[i] - k, 0);
            hashMap.put(prefix[i], hashMap.getOrDefault(prefix[i], 0) + 1);
        }
...
- **更新顺序**：必须先查 `curSum - k`，再将当前 `curSum` 存入哈希表。如果先存再查，当 k=0 时每个位置都会把自己算进去（把空子数组也算上了）。
...
public int subarraySum(int[] nums, int k) {
    ...
    for (int num : nums) {
        ...
        // Step 3: 查哈希表 -- 找有多少个前缀和等于 curSum - k
...
- [[525-连续数组]] -- 将 0/1 转为 plusmn1，再用前缀和+哈希表找和为 0 的最长连续子数组。
