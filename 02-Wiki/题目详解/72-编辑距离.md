        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                if (word1.charAt(i - 1) == word2.charAt(j - 1)) {
                    ...
                    dp[i][j] = Math.min(
                        Math.min(dp[i - 1][j] + 1,        // 删除 word1[i-1]
...
class Solution {
    public int minDistance(String word1, String word2) {
        int m = word1.length(), n = word2.length();
        int[] dp = new int[n + 1];               // 初始化第 0 行：0, 1, 2, ..., n
        for (int j = 0; j <= n; j++) {
            dp[j] = j;
        }
        
        for (int i = 1; i <= m; i++) {
            int prev = dp[0];                      // 左上角 dp[i-1][j-1]
            dp[0] = i;                            // 每行第一个：dp[i][0] = i
            for (int j = 1; j <= n; j++) {
                int temp = dp[j];                  // 保存当前值（将成为下一列的左上角）
                if (word1.charAt(i - 1) == word2.charAt(j - 1)) {
                    dp[j] = prev;                 // 相同，无需操作
                } else {
                    dp[j] = Math.min(
                        Math.min(dp[j] + 1,        // 删除（来自上方）
                                 dp[j - 1] + 1),   // 插入（来自左方）
                        prev + 1                   // 替换（来自左上角）
                    );
                }
...
class Solution {
    ...
    private int dfs(int i, int j, String w1, String w2, int[][] memo) {
        if (i == 0) return j;                      // word1 为空，插入所有
        if (j == 0) return i;                      // word2 为空，删除所有
        if (memo[i][j] != -1) return memo[i][j];
        
        if (w1.charAt(i - 1) == w2.charAt(j - 1)) {
            memo[i][j] = dfs(i - 1, j - 1, w1, w2, memo);  // 字符相同，跳过
        } else {
            memo[i][j] = Math.min(
                Math.min(dfs(i - 1, j, w1, w2, memo) + 1,    // 删除
                         dfs(i, j - 1, w1, w2, memo) + 1),   // 插入
                dfs(i - 1, j - 1, w1, w2, memo) + 1          // 替换
            );
        }
        return memo[i][j];
    }
...
- **字符串索引偏移**：`word1.charAt(i-1)` 与 `word2.charAt(j-1)` 比较，不是 `word1.charAt(i)` 与 `word2.charAt(j)`。
