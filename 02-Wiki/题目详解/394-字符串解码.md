...
class Solution {
    public String decodeString(String s) {
        Deque<Object[]> stack = new ArrayDeque<>();  // 存储 [prevStr, num]
        StringBuilder curStr = new StringBuilder();  // 当前正在构建的字符串
        int curNum = 0;                              // 当前累积的数字
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (Character.isDigit(ch)) {
                ...
                stack.push(new Object[]{curStr.toString(), curNum});  // 保存当前状态
                curStr = new StringBuilder();  // 重置，开始处理内层
                curNum = 0;                    // 重置数字
            } else if (ch == ']') {
                Object[] prev = stack.pop();   // 取出外层状态
                String prevStr = (String) prev[0];
                int num = (Integer) prev[1];
                curStr = new StringBuilder(prevStr + curStr.toString().repeat(num));  // 拼接：外层 + 内层×次数
            } else {  // 字母
                curStr.append(ch);
            }
        }
...
class Solution {
    public String decodeString(String s) {
        ...
        Deque<String> strStack = new ArrayDeque<>();      // 字符串栈
        int curNum = 0;
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (Character.isDigit(ch)) {
                ...
                curStr = new StringBuilder(strStack.pop() + curStr.toString().repeat(numStack.pop()));
            } else {
                curStr.append(ch);
            }
        }
    ...
    private String dfs(String s, int[] idx) {
        ...
        while (idx[0] < s.length()) {
            ...
            if (Character.isDigit(ch)) {
                ...
                idx[0]++;
                String innerStr = dfs(s, idx);   // 递归处理内层
                res.append(innerStr.repeat(num));
                num = 0;
            } else if (ch == ']') {
                return res.toString();
            } else {  // 字母
...
- **多位数处理：** 数字可能不止一位（如 `12[a]`），需要用 `curNum = curNum * 10 + (ch - '0')` 累积，而不是直接赋值。忘记处理多位数是最常见的 bug。
- **嵌套重置：** 遇到 `[` 时需要重置 `curStr` 和 `curNum`，否则内层会错误地继承外层的状态。
- **拼接顺序：** `prevStr + curStr.toString().repeat(num)` — 外层的字符串应该在前，内层重复后的字符串在后。顺序搞反会导致结果错误（如 `3[a2[c]]` 会得到 "ca" × 3 而不是 "acc" × 3）。
...
class Solution {
    public String decodeNestedString(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (Character.isDigit(ch)) {
                ...
                stack.push(new Object[]{curStr.toString(), curNum});
                curStr = new StringBuilder();
                curNum = 0;
            } else if (ch == ']') {
                ...
                String prevStr = (String) prev[0];
                int num = (Integer) prev[1];
                curStr = new StringBuilder(prevStr + curStr.toString().repeat(num));
            } else {
                curStr.append(ch);
            }
        }
