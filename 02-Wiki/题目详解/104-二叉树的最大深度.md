...
- **数据范围含义：** 节点数最多 10000，递归深度可能达到 10000，某些语言可能需要考虑栈溢出（Python 默认递归深度约 1000，可能不够）。Java 的递归栈同样受限，建议用迭代法或考虑极端情况。
- **输入输出特征：** 输入为根节点，输出为整数（深度）。根节点自身算 1，空树返回 0。
- **边界条件：** `root = null` 返回 0（空树深度为 0）；单节点返回 1。
...
- **空树返回 0 而不是 null：** base case 必须返回 0，很多初学者会忘记这个特判。
...
int traverse(TreeNode node) {
    ...
    int leftResult = traverse(node.left);    // 左子树结果
    int rightResult = traverse(node.right);  // 右子树结果
    // 后序位置：用左右子树的结果构造当前节点的答案
    return compute(node, leftResult, rightResult);
}
```
