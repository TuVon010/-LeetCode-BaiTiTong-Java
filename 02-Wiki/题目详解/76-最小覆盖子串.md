...
**数据范围含义：**
- s 最大 10^5，t 最大 10^5。暴力法——枚举所有子串 O(n^2 x m) 完全不可行。
- 需要 O(n) 或 O(n log n) 的解法。
- 字符集大小写敏感，意味着 'A' 和 'a' 是不同的字符。总共有 26x2 = 52 个可能的字符（仅限字母），但为了通用性，用哈希表而非固定数组更稳妥。
...
- **检查是否找到结果：** 注意 `length` 的初始值应该是 `Integer.MAX_VALUE`（或 `len(s) + 1`），返回时检查 `length` 是否被更新过。不能用 `0` 或 `len(s)` 作为未找到的标志（因为可能真的有一个长度为 0 或 len(s) 的子串）。
- **t 中可能有重复字符：** `need` 字典记录的是每个字符的需求数量，不是种类数。`valid` 的最大值是 `need.size()`（不同种类数），不是 `t.length()`。
...
```java
public String slidingWindowComplex(String s, String target) {
    // ===== 1. 初始化 =====
    Map<Character, Integer> need = new HashMap<>();      // 目标需求
    for (char ch : target.toCharArray()) {
        need.put(ch, need.getOrDefault(ch, 0) + 1);
    }
    
    Map<Character, Integer> window = new HashMap<>();    // 窗口计数
    int valid = 0;                    // 已满足的「条件」数量
    int left = 0;
    
    // 结果记录
    int resultStart = 0;
    int resultLength = Integer.MAX_VALUE;  // 找最短用 MAX_VALUE，找最长用 0
    ...
    for (int right = 0; right < s.length(); right++) {
        ...
        if (need.containsKey(val)) {
            ...
            if (window.get(val).equals(need.get(val))) {
                valid++;  // 该条件已满足
            }
        }
...
|------|--------------|------------------|
| resultLength 初值 | `Integer.MAX_VALUE` | `0` |
| 何时更新答案 | 缩小窗口前（此时满足条件且最小） | 扩大窗口后（此时满足条件且最大） |
| 收缩条件 | `valid == need.size()` | `valid < need.size()`（不满足时收缩） |
